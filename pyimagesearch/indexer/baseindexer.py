...
class BaseIndexer(object):
	def __init__(self, dbPath, estNumImages = 500, maxBufferSize=5000,dbResizeFactor=2,
		verbose = True):
		# store thedatabase path, estimated number of images in the dataset, max
	...
	def _writeBuffer(self, dataset, datasetName, buf, idxName, sparse= False):
		...
		# check to see if dataset needto be resized
		if end > dataset.shape[0]:
			self._debug("triggering '{}' db resize".format(datasetName))
			self._resizeDataset(dataset, datasetName, baseSize=end)
	
		# if there is a sparse matrix, then convert the sparse matrix to a 
		# dense one so it can be written to a file
		if sparse:
			buf=buf.toarray()

		# dumpthe buffer to a file
		self._debug("writting '{}' buffer".format(datasetName))
		dataset[self.idxs[idxName]:end] = buf

	def _resizeDataset(self, dataset,dbName,baseSize=0,finished=0):
		# grab the original set of dataset
	...
	def featureStack(array, accum=None, stackMethod=np.vstack):
		...
		# otherwise stack the array
		else:
			accum = stackMethod([accum, array])

		# retuen the accumulated array
		return accum
