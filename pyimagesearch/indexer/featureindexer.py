...
from pyimagesearch.indexer import baseindexer
import numpy as np
import h5py
import sys

class FeatureIndexer(baseindexer.BaseIndexer):
	...
	def add(self, imageID, kps, features):
		# compute the start and end index of features lookup
		start = self.idxs["features"] + self.totalFeatures
		end = start + len(features)

		# update the image ID buffer, features buffer, and index buffer
		# followed by incrementing the feature count
		self.imageIDBuffer.append(imageID)
		self.featuresBuffer = baseindexer.BaseIndexer.featureStack(np.hstack([kps, features]),
			self.featuresBuffer)
		...
		if self.totalFeatures >= self.maxBufferSize:
			# if the databases is not been created yet create them
	...
	def _createDatasets(self):
		...
		# and use this number to determine the appropriate number of features
