...
class Vocabulary:
	def __init__(self,dbPath,verbose=True):
		# store the database path and verbosity setting
		self.dbPath=dbPath
		self.verbose=verbose

	def fit(self,numClusters, samplePercentage, randomState=None):
		...
		# sample, sorting them in ascending order to speedup access time from
		# the HDF5 database
		sampleSize = int(np.ceil(samplePercentage * totalFeatures))
		idxs = np.random.choice(np.arange(0,totalFeatures),(sampleSize),replace=False)
		idxs.sort()
		data=[]
		self._debug("starting sampling ...")

		# loop over randomly sampled indexes and accumulate the 
		#features to cluster
		for i in idxs:
