...
print("[INFO] starting distance computation ...")

# initialize the visaulize dictionary and initialize the progressbar
...
for (i,imageID) in enumerate(featuresDB["image_ids"]):
	...
	# loop over each of the individual keypointsand feature vectors
	for (kp,features) in zip(kps,descs):
		...
		# loop over distances dictionary
...
for (vwID, results) in vis.items():
	...
	montage = resultsmontage.ResultsMontage((64, 64), 4, 16)
